...
class Scratch:
        ...
        def _callback(response):
            ...
            try:
                ...
                if not outputs or len(outputs) < 5:
                    print(f"⚠️ schedule_daily got {len(outputs) if outputs else 0} schedules (need >=5), using failsafe")
                    return failsafe
    ...
    def prompt_generate_chat(self, agent, other, relation, chats):
        ...
        if chats:
            ...
            conversation = "[对话尚未开始，请主动开启话题]"

        address = agent.get_tile().get_address()
        curr_context = (
            f"{agent.name} 正在 {agent.get_event().get_describe(False)}，"
            f"看到 {other.name} 正在 {other.get_event().get_describe(False)}。"
        )
        ...
        def _callback(response):
            ...
            novlang = novlang.replace("\n\n", "\n").strip(' \n"\'')
            
            chinese = json_content.get("chinese", "") or json_content.get("translation", "")
            chinese = chinese.replace("\n\n", "\n").strip(' \n"\'')
            
            scene_observation = json_content.get("scene_observation", "")
            thinking = json_content.get("thinking", "")
    ...
    def prompt_understand_novlang(self, listener, speaker, novlang_text, scene_context, conversation_history):
        """让listener尝试理解speaker说的Novlang"""
        prompt = self.build_prompt(
            "understand_novlang",
            {
                "listener": listener,
                "speaker": speaker,
                "novlang_text": novlang_text,
        ...
        def _callback(response):
            ...
            return {
                ...
                "context_clues": json_content.get("context_clues", ""),
                "my_understanding": json_content.get("my_understanding", ""),
            }

        return {
            "prompt": prompt,
            "callback": _callback,
            "failsafe": {"symbol_analysis": "", "context_clues": "", "my_understanding": "我不太理解"},
        }

    def prompt_verify_understanding(self, speaker, listener, original_meaning, novlang_text, listener_understanding):
        """验证listener是否正确理解了speaker的意思"""
        prompt = self.build_prompt(
        ...
        def _callback(response):
            ...
            return {
                ...
                "emotion_match": json_content.get("emotion_match", "不一致"),
                "intent_match": json_content.get("intent_match", "不一致"),
                "overall_score": int(json_content.get("overall_score", 5)),
