...
class Agent:
    ...
    def _extract_chat_content(self, response):
        """提取对话内容，支持新旧格式"""
        if response is None:
            return {"novlang": "", "chinese": "", "scene_observation": "", "thinking": ""}
        if isinstance(response, dict):
            # 新格式：novlang, chinese, scene_observation, thinking
            # 兼容旧格式：text, translation
            novlang = response.get("novlang", "") or response.get("text", "") or ""
            chinese = response.get("chinese", "") or response.get("translation", "") or ""
            scene_observation = response.get("scene_observation", "") or ""
            thinking = response.get("thinking", "") or ""
            
            novlang = novlang.strip() if isinstance(novlang, str) else str(novlang).strip()
            chinese = chinese.strip() if isinstance(chinese, str) else str(chinese).strip()
            scene_observation = scene_observation.strip() if isinstance(scene_observation, str) else str(scene_observation).strip()
            thinking = thinking.strip() if isinstance(thinking, str) else str(thinking).strip()
    ...
    def _chat_with(self, other, focus):
        ...
        start, raw_chats = utils.get_timer().get_date(), []
        display_chats = []
        relations = [
        ...
        for i in range(self.chat_iter):
            response = self.completion(
                "generate_chat", self, other, relations[0], raw_chats
            )
            text, translation, understanding, thinking = self._extract_chat_content(response)

            if i > 0:
                # 对于发起对话的Agent，从第2轮对话开始，检查是否出现“复读”现象
                end = self.completion(
                    "generate_chat_check_repeat", self, raw_chats, text
                )
                if end:
                    break

                # 对于发起对话的Agent，从第2轮对话开始，检查话题是否结束
                raw_chats.append((self.name, text))
                display_chats.append([self.name, text, translation, understanding, thinking])
                end = self.completion(
                    "decide_chat_terminate", self, other, raw_chats
                )
                if end:
                    break
            else :
            ...
            response = other.completion(
                "generate_chat", other, self, relations[1], raw_chats
            )
            text, translation, understanding, thinking = self._extract_chat_content(response)
            if i > 0:
                # 对于响应对话的Agent，从第2轮开始，检查是否出现“复读”现象
                end = self.completion(
                    "generate_chat_check_repeat", other, raw_chats, text
                )
                if end:
                    break

            raw_chats.append((other.name, text))
            display_chats.append([other.name, text, translation, understanding, thinking])

            # 对于响应对话的Agent，从第1轮开始，检查话题是否结束
            end = other.completion(
                "decide_chat_terminate", other, self, raw_chats
            )
            if end:
                break
        ...
        self.conversation[key].append(
            {
                f"{self.name} -> {other.name} @ {'，'.join(self.get_event().address)}": display_chats
            }
        )
                ...
                "\n  ".join(
                    [
                        "{}: {}{}\n    [理解: {}] [思考: {}]".format(
                            chat[0],  # name
                            chat[1],  # text
                            f" [翻译：{chat[2]}]" if len(chat) > 2 and chat[2] else "",
                            chat[3] if len(chat) > 3 else "",
                            chat[4] if len(chat) > 4 else "",
                        )
        ...
        chat_summary = self.completion("summarize_chats", raw_chats)
        duration = int(sum([len(c[1]) for c in raw_chats]) / 240)
        self.schedule_chat(
            raw_chats, chat_summary, start, duration, other
        )
        other.schedule_chat(raw_chats, chat_summary, start, duration, self)
        return True
