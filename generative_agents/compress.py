                    ...
                    for chats in conversation[step_time]:
                        for persons, chat in chats.items():
                            ...
                            for c in chat:
                                agent = c[0]
                                text = c[1]
                                translation = c[2] if len(c) > 2 else ""
                                display_text = text
                                if translation:
                                    display_text = f"{display_text} [翻译：{translation}]"
                                step_conversation += f"{agent}：{display_text}\n"
    ...
    def extract_action(json_data):
        ...
        for chats in conversation[json_data['time']]:
            for agents, chat in chats.items():
                ...
                for item in chat:
                    text = item[1]
                    translation = item[2] if len(item) > 2 else ""
                    if translation:
                        text = f"{text} [翻译：{translation}]"
